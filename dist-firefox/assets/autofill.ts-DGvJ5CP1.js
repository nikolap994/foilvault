...
function removeFillUI() {
  fillBtn == null ? void 0 : fillBtn.remove();
  fillBtn = null;
  fillMenu == null ? void 0 : fillMenu.remove();
  fillMenu = null;
}
function showFillButton(pwField) {
  removeFillUI();
  ...
  chrome.runtime.sendMessage({ type: "autofill-get-credentials", hostname: location.hostname, url: location.href }, (res) => {
    ...
    for (const c of creds) {
      ...
      row.addEventListener("mouseenter", () => {
        row.style.background = "rgba(129,140,248,0.08)";
      });
...
chrome.runtime.onMessage.addListener((msg) => {
  if ((msg == null ? void 0 : msg.type) !== "foilvault-context-fill") return;
...
async function init() {
  ...
  const enabled = ((_a = stored.foilvault_options) == null ? void 0 : _a.autofillEnabled) ?? true;
  if (!enabled) return;
...
init();
//# sourceMappingURL=autofill.ts-DGvJ5CP1.js.map
})()
